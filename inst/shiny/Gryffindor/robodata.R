...
data_scrap <- function(stocks,industries,country, yrs, scraping_function){
  for (item in Map(list, stocks,industries)){
    ...
    df <- scraping_function(item[[1]],
                            country = country,
                            from_date = format((Sys.Date() - years(yrs)),
                                               '%d/%m/%Y'),
                            to_date = format(Sys.Date(),
                                             '%d/%m/%Y'), as_json = TRUE)
    #df$industry <- rep(item[[2]], nrow(df))


    df <- jsonlite::fromJSON(df) %>% as.data.frame() %>%
        subset(select = c('historical.date','historical.close'))
...
EU <- data_scrap(ind_stocks_eu, inds_eu, 'Germany', 15,
                 invest$get_etf_historical_data) %>%
    reduce(inner_join, by = "Date")
US <- data_scrap_ltd(ind_stocks_us, inds_us, 'world',
                     invest$get_index_historical_data) %>%
    reduce(inner_join, by = "Date")
...
Africa <- as.data.frame(ds.getSymbol.yahoo("EZA", from = "2006-12-08",
                                           to = Sys.Date())[,6])
Australia <- as.data.frame(ds.getSymbol.yahoo("EWA", from = "2006-12-08",
                                              to = Sys.Date())[,6])
Latinamerica <- as.data.frame(ds.getSymbol.yahoo("ILF", from = "2006-12-08",
                                                 to = Sys.Date())[,6])
Commodities <- as.data.frame(ds.getSymbol.yahoo("IAU", from = "2006-12-08",
                                                to = Sys.Date())[,6])
LongBond <- as.data.frame(ds.getSymbol.yahoo("TLT", from = "2006-12-08",
                                             to = Sys.Date())[,6])
ShortBond <- as.data.frame(ds.getSymbol.yahoo("TIP", from = "2006-12-08",
                                              to = Sys.Date())[,6])
# Data for Benchmark
sp500 <- as.data.frame(ds.getSymbol.yahoo("^GSPC", from = "2006-12-08",
                                          to = Sys.Date())[,6])
stoxx <- as.data.frame(ds.getSymbol.yahoo("FEZ", from = "2006-12-08",
                                          to = Sys.Date())[,6])
asia <- as.data.frame(ds.getSymbol.yahoo("VPL", from = "2006-12-08",
                                         to = Sys.Date())[,6])
...
for (c in (1:(length(benchmarklist) - 1))) {
  ...
  if(c == 1){
    ...
    databenchmark <- merge.data.frame(databenchmark, benchmarklist[c + 1],
                                      by = "row.names")
...
benchmark <- as.data.frame(matrix(data = NA, ncol = 1,
                                  nrow=nrow(databenchmark)))
weights <- c(0.4, 0.3, 0.3)

for (r in 1:nrow(databenchmark)) {
  benchmark[r, 1] <- as.numeric(as.matrix(databenchmark[r, ]) %*%
                                   as.matrix(weights))
}

rownames(benchmark) <- rownames(sp500)

indices = c("Africa", "Australia", "Latinamerica", "Commodities", "LongBond",
            "ShortBond", "Benchmark")
dflist <- list(Africa, Australia, Latinamerica, Commodities, LongBond,
               ShortBond, benchmark)
